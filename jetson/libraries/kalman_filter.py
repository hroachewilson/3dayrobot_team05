...
class Kalman:
    ...
    # when you get a new observation — someNewPoint = np.r_[1,2]
	...
	# Process noise
        self.Q = np.matrix(np.eye(state_dim)*1e-4)

	# Observation noise
        self.R = np.matrix(np.eye(obs_dim)*0.01)

	# Transition matrix
        self.A = np.matrix(np.eye(state_dim))

	# Measurement matrix
        self.H = np.matrix(np.zeros((obs_dim, state_dim)))

	# Gain matrix
        self.K = np.matrix(np.zeros_like(self.H.T))

	# State covariance
        self.P = np.matrix(np.zeros_like(self.A))

        # The actual state of the system
        self.x = np.matrix(np.zeros((state_dim, 1)))
